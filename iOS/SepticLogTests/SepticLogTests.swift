import XCTest
@testable import SepticLog

final class SepticLogTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testAddEntrySucceedsUnderLimit() {
        ...
        let added = store.add(EntryEntry(), isPro: false)
        XCTAssertTrue(added)
        XCTAssertEqual(store.entries.count, before + 1)
    }

    @MainActor func testAddEntryFailsAtFreeLimit() {
        store.entries = (0..<Store.freeLimit).map { _ in EntryEntry() }
        let added = store.add(EntryEntry(), isPro: false)
    ...
    @MainActor func testProUserBypassesLimit() {
        store.entries = (0..<Store.freeLimit).map { _ in EntryEntry() }
        let added = store.add(EntryEntry(), isPro: true)
        XCTAssertTrue(added)
    }
    ...
    @MainActor func testDeleteAtOffsets() {
        store.entries = [EntryEntry(), EntryEntry()]
